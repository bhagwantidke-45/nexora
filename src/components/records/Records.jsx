...
const Records = () => {
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-mesh" : "bg-mesh-light"}`}>
      ...
      <div className="pt-20 pb-8 px-4 max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
            {filtered.map((record) => {
              if (record.type === "note") return <NoteCard key={record.id} record={record} onEdit={handleEdit} onDelete={handleDelete} onPin={handlePin} />;
              if (record.type === "contact") return <ContactCard key={record.id} record={record} onEdit={handleEdit} onDelete={handleDelete} onPin={handlePin} />;
              if (record.type === "date") return <DateCard key={record.id} record={record} onEdit={handleEdit} onDelete={handleDelete} onPin={handlePin} />;
              if (record.type === "file") return <FileCard key={record.id} record={record} onEdit={handleEdit} onDelete={handleDelete} onPin={handlePin} />;
              return null;
            })}
          </div>
        )}
      ...
      {/* Record Form Modal */}
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditRecord(null); }} title={editRecord ? "Edit Record" : "Add Record"} size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Record title..." className="input-glass" required />
          </div>
          ...
          {form.type === "note" && (
            <div>
              ...
              <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} placeholder="Write your note..." rows={4} className="input-glass resize-none" />
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="pinned" checked={form.pinned} onChange={(e) => setForm({ ...form, pinned: e.target.checked })} className="rounded" />
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => { setShowForm(false); setEditRecord(null); }} className="btn-secondary flex-1 justify-center">Cancel</button>
