...
import { Timer, Play, Pause, RotateCcw, Coffee, Brain, Music, Volume2, VolumeX, Settings, Zap, CheckCircle } from "lucide-react";
...
const Focus = () => {
  ...
  const [totalFocus, setTotalFocus] = useState(0); // seconds focused today
        ...
        setSeconds(s => {
          if (s <= 1) {
            ...
            if (mode === "pomodoro" || mode === "deep") {
              ...
              setCompletedSessions(cs => [...cs, { mode, duration: total, time: new Date().toLocaleTimeString() }]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 glass-card p-8 flex flex-col items-center animate-scale-in">
              ...
              {Object.entries(MODES).map(([k, v]) => (
                <button key={k} onClick={() => setMode(k)}
                  className={`px-4 py-1.5 rounded-xl text-sm font-medium transition-all ${
                    mode === k ? "text-white shadow-lg" : "glass-card text-gray-400 hover:text-white"
                  }`}
                  style={mode === k ? { background: `linear-gradient(135deg, ${v.color}99, ${v.color}66)`, border:`1px solid ${v.color}44` } : {}}>
                  {v.label}
                </button>
              ))}
            </div>
            ...
            <div className="relative mb-6 animate-float" style={{ animationDuration:"4s" }}>
              <Ring pct={pct} color={cfg?.color || "#a855f7"} />
              {/* Glow */}
              <div className="absolute inset-0 rounded-full pointer-events-none"
                style={{ boxShadow: running ? `0 0 60px ${cfg?.color || "#a855f7"}44` : "none", transition:"box-shadow 0.5s ease", borderRadius:"50%" }} />
              {/* Center */}
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-display font-bold text-5xl text-white tracking-tight animate-count-up">
                  {fmt(seconds)}
                </span>
                <span className="text-gray-400 text-sm mt-1">{cfg?.label}</span>
                {running && (
                  <span className="flex items-center gap-1 text-xs mt-2 animate-pulse-slow" style={{ color: cfg?.color }}>
                    <Zap size={10} /> In flow
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={toggle}
                className="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-xl font-bold shadow-lg transition-all hover:scale-110 active:scale-95"
                style={{ background:`linear-gradient(135deg, ${cfg?.color || "#a855f7"}, ${cfg?.color || "#a855f7"}99)`, boxShadow:`0 8px 30px ${cfg?.color || "#a855f7"}44` }}>
            ...
            <div className="w-full mt-6">
              ...
              <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-500"
                  style={{ width:`${pct}%`, background:`linear-gradient(90deg, ${cfg?.color||"#a855f7"}, ${cfg?.color||"#a855f7"}88)` }} />
              </div>
            </div>
          </div>
              ...
              <div className="flex gap-2">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className={`flex-1 h-3 rounded-full transition-all duration-500 ${
                    i < sessions % 4 ? "bg-primary-500 glow-primary" : "bg-white/10"
                  }`} />
                ))}
              </div>
              <p className="text-gray-500 text-xs mt-2">{sessions % 4}/4 — {sessions >= 4 ? "Take a long break!" : "Keep going!"}</p>
            </div>
            ...
            {showSettings && (
              <div className="glass-card p-5 animate-scale-in">
                ...
                <div>
                  ...
                  <button onClick={() => { setMode("custom"); setSeconds(customMin * 60); setShowSettings(false); }}
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                  {[...completedSessions].reverse().slice(0,8).map((s, i) => (
                    <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-white/5 animate-fade-in">
                      <span className="text-gray-300 text-xs capitalize">{s.mode} · {Math.round(s.duration/60)}min</span>
